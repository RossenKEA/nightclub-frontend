...
export default async function EventPage({ params }: Props) {
  ...
  return (
    <main className="bg-black text-white">
      {/* Top title banner */}
      <section className="relative flex h-36 items-center justify-center bg-[url('/images/event-banner.jpg')] bg-cover bg-center">
        <div className="absolute inset-0 bg-black/60" />
        <h1 className="relative z-10 text-center text-3xl font-bold uppercase tracking-[0.15em]">
  ...
  />
        ...
        <div className="grid grid-cols-2 gap-y-8 border-b border-white/40 pb-10 text-center md:grid-cols-4">
          ...
          <Info label="Doors Open:" value={event.doorsOpen} />
          <Info label="Start:" value={event.startTime} />
          ...
          <Info label="Entrance:" value={`${event.price}KR`} />
          <Info label="Scene:" value={event.scene} />
        </div>
        ...
        <section className="py-20 text-center">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4 text-2xl font-bold uppercase tracking-[0.15em] text-pink-500">
        ...
        <section className="mb-20 text-center">
          ...
          <div className="mx-auto mt-10 max-w-2xl rounded-lg border border-pink-500 p-8 text-left">
        ...
        <section className="mx-auto max-w-2xl text-center">
          ...
          <Link
            ...
            className="mt-12 inline-block border-y border-white px-8 py-4 font-bold uppercase tracking-widest hover:text-pink-500"
          >
            Book Now
          </Link>
        </section>
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <h3 className="font-bold uppercase text-pink-500">{label}</h3>
      <p>{value}</p>
    </div>
  );
}
